...
# Supervisor: mantiene "npm run dev" corriendo indefinidamente, reiniciandolo
...
$rutaTrabajo = "C:\Claude_2026\Proyectos\07 Tienda en Linea Kukis\frontend"
...
$logSupervisor = Join-Path $carpetaLogs "supervisor_KukisFrontend.log"
$logCrashes = Join-Path $carpetaLogs "crashes_KukisFrontend.log"
...
Escribir-Log "Supervisor iniciado (KukisFrontend)."

while ($true) {
    Escribir-Log "Iniciando: npm run dev (puerto 5176)..."
    ...
    $lineaComando = "npm run dev 2>> `"$logCrashes`""
    cmd /c $lineaComando
    $codigo = $LASTEXITCODE
